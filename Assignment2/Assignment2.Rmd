...
### Output : 
The Decision Curve Analysis plot, with the default stratagies and our chosen model (predicted_risk).
...
Then in range 3-25% the model's net benefit surpasses the "treat-all" strategy, staying constantly above the default strategies. This suggests its clinical usefullnes in this threshold, as it means the model strikes a good balance of the trade-off between correctly identifying patients at risk and avoiding unnecessary treatments.
...
### Code:
```{r echo=FALSE}
	dca_data_combined <- data.frame(predicted_risk_agegroup = agegroup_probs, predicted_risk = di$predicted_risk, actual_label = di$chd69)
	dca_combined <- dca(actual_label ~ predicted_risk + predicted_risk_agegroup, dca_data_combined)
	options(repr.plot.width=16, repr.plot.height=9)
	plot(dca_combined)
```
...
To cnoclude it is only slightly more useful than the default strategies and less useful than the final model. 
